...
An alternative to grids is using tree-like structures. Metacity supports exporting data into [quad-trees](https://en.wikipedia.org/wiki/Quadtree):
...
The example above constructs a quad-tree on top of the layer passed in the constructor. The output directory is going to contain the following files:
...
```json5
{
    ...
    "file": "quad5_345.glb",
    "size": 245,
    //node metadata, aggregated from models containd in the node
    "metadata": {
        "type": "building",
        "xyz": 567
    },
...
Similarly to the grid tile-mergning modifier, it is possible to merge all models in a particular tree level:
