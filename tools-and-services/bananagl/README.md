...
| `onClick`                      |     -    | `(id: number, metadata: Object) => string`, attach a callback that will return a string to be placed on top of an object after a click; requires `pickable` to be set up to true on layers where the callback should be applied |
